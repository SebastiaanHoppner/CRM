...
for (k_value in k_seq) {
  ...
  results_MAE_k       <- data.frame(matrix(nrow = n_sims, ncol = 5))
  results_MSEP_k      <- data.frame(matrix(nrow = n_sims, ncol = 5))
  results_RMSEI_k     <- data.frame(matrix(nrow = n_sims, ncol = 2))
...
results_precision <- cbind.data.frame(do.call(rbind, results_precision), k = sort(rep(k_seq, n_sims)))
results_recall    <- cbind.data.frame(do.call(rbind, results_recall),    k = sort(rep(k_seq, n_sims)))
